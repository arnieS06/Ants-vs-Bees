...
# Name: Arnav Sharma, Glenn Mah
# Email: 30069505@cc.peralta.edu, 30001181@cc.peralta.edu
...
class Place:
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            elif isinstance(insect,QueenAnt) and insect.imposter == False:
                return
            else:
                self.ant = None
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        self.armor = 1
        deal_damage = self.place.bees[:]
        for bees in deal_damage:
            bees.reduce_armor(self.damage)
...
class QueenAnt(ScubaThrower):
    ...
    implemented = True
    food_cost = 6
    queen_placed = False  #  will track whether a true queen has been placed
    ...
    def action(self, colony):
        ...
        # checking queen ant place first 
        ant = self.place.ant
        if ant and ant is not self:
            if ant not in self.buffed_ants:
                ant.damage *= 2
                self.buffed_ants.append(ant)
            if ant.container and ant.ant and ant.ant not in self.buffed_ants:
                ant.ant.damage *= 2
                self.buffed_ants.append(ant.ant)

        # checking with direction because auto grader could not find buffs on bee

        for direction in ['exit', 'entrance']:
            current = getattr(self.place, direction)
